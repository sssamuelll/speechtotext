...
**(b) ✅ hecho a nivel de lógica** (`speakers/diarization.py`, pendiente verificar contra el audio real): `assign_segments` asigna hablante por **palabra** (`word_timestamps=True` sólo al diarizar) y parte cada segmento de Whisper en los cambios de hablante internos — antes etiquetaba el segmento entero con un solo hablante y arrastraba la cola. Casos borde cubiertos con tests tras dos rondas de verificación adversarial: palabra en hueco entre turnos / de duración 0 hereda el hablante del run (no sale como `Hablante ?`); `_best_speaker` agrega solape **por hablante** (gana el mayoritario aunque pyannote fragmente su voz en varios turnos). **Watch-item:** la hipótesis secundaria (desfase sistemático whisper↔pyannote por `vad_filter`) no se verificó — si al correr sobre el audio real la cola sigue arrastrada de forma uniforme, el culpable es ese desfase, no la granularidad.
